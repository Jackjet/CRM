            ...
            try
            {
                ...
                if (edit_linkman != null)
                {
                    ...
                    new Thread(() =>
                    {
                        ...
                        string[] link_Users = Split_Hepler.str_to_stringss(edit_linkman.link_users);
                        string[] link_usersnames = Split_Hepler.str_to_stringss(edit_linkman.link_usersname);

                        cust_customer customer_edit = cust_customer_handle.list_All.FirstOrDefault(i => i.id == edit_linkman.link_cust_id);


                        //先去除客户信息
                        //string[] cust_Users = Split_Hepler.str_to_stringss(customer_edit.cust_users);
                        ...
                        {
                            for (int i = 0; i < link_Users.Count(); i++)
                            {
                                if (!customer_edit.cust_users.Contains(link_Users[i]))
                                {
                                    customer_edit.cust_users += "," + link_Users[i];
                                    customer_edit.cust_usersname += "," + link_usersnames[i];
                                }
                            }
                        }

                        cust_customer_handle.Add_Other_Users(customer_edit, cust_leaders);

                        jsonModel = new cust_customerService().Update(customer_edit);
